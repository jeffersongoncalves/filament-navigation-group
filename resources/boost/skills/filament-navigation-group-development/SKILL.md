...
`NavigationManager` clones every `NavigationItem` twice before ordering them
(once in its constructor, once again inside `get()` itself), which would
otherwise drop the `groupSort()` value tracked in the `WeakMap` (a clone has
a new object identity). `ExtendedNavigationManager` re-attaches it at both
clone boundaries.
